  ...
  )
{
  ...
  for (lMemoryIndex = Dem_MemState_GetFirstValidEventBlockIndex(); lMemoryIndex < lMemoryIndexEnd; ++lMemoryIndex)
  {
    ...
    lEventId = Dem_MemoryEntry_GetEventId(Dem_MemState_GetMemoryEntryId(lMemoryIndex));
    if (Dem_Event_TestValidHandle(lEventId) == TRUE)
    {
      ...
      {
        boolean lDirty;
        lDirty = Dem_MemStorageManager_TestCycleCounterLatched(Dem_MemState_GetMemoryEntryId(lMemoryIndex), CycleCounter);
        if (lDirty == TRUE)
        {
  ...
  )
{
...
#if (DEM_CFG_SUPPORT_OBDII == STD_ON)
  lOldPermanentCount = Dem_MemState_GetNumAllocatedMemoryEntry(DEM_CFG_MEMORYID_PERMANENT);
#endif
  ...
  if ( (lOldPermanentCount == Dem_Cfg_GlobalPermanentMaxCount())                                                                 /* PRQA S 3415 */ /* MD_DEM_12.4_cf */
     && (lOldPermanentCount > 
         Dem_MemState_GetNumAllocatedMemoryEntry(DEM_CFG_MEMORYID_PERMANENT)) )
