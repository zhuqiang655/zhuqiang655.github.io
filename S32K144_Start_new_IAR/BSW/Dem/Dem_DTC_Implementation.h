...
#if (DEM_DEV_RUNTIME_CHECKS == STD_ON)
      if (Dem_MemoryEntry_GetAgingCounter(lMemoryEntryId) != DEM_OPERATIONCYCLE_CYCLECOUNT_INVALID)
      {
        Dem_Error_RunTimeCheckFailed(__FILE__, __LINE__);                                                                        /* SBSW_DEM_POINTER_RUNTIME_CHECK */
      }
#endif
...
)
{
  ...
  if (Dem_DTCInternalStatus_GetStoredStatus(lInternalStatus) != Dem_DTCInternalStatus_StoredStatus_None)
  {
    ...
    lMemoryInfo = Dem_MemState_MemoryInfoInit(Dem_Cfg_EventDestination(DTCContext->EventId));
    lMemoryIndex = Dem_MemAccess_MemoryFindIndex_Checked(lMemoryInfo, DTCContext->EventId);                                      /* SBSW_DEM_CALL_MEMORYINFO_POINTER */
    lMemoryEntryId = Dem_MemState_GetMemoryEntryId(lMemoryIndex);
  ...
  )
{
  ...
  while (lChronoIndex != 0)
  {
    ...
    lMemoryIndex = Dem_MemState_MemoryGetChronology(Dem_MemState_MemoryInfoInit(DEM_CFG_MEMORYID_PRIMARY), lChronoIndex);        /* SBSW_DEM_CALL_MEMORYINFO_POINTER */
    lMemoryEntryId = Dem_MemState_GetMemoryEntryId(lMemoryIndex);
...
)
{
  ...
  { /* passive event */
    ...
    { /* Untested cycle requires recalculation of aging target */
      uint16 lCycleCount = Dem_MemoryEntry_GetAgingCounter(Dem_MemState_GetMemoryEntryId(MemoryIndex));
      ...
      { /* Ignore maxed (latched) counter */
        ...
        Dem_MemoryEntry_SetAgingCounter(Dem_MemState_GetMemoryEntryId(MemoryIndex), lCycleCount);
...
#endif
    if (CycleCounter == Dem_MemoryEntry_GetAgingCounter(Dem_MemState_GetMemoryEntryId(MemoryIndex)))
  ...
  )
{
  if (Dem_Cfg_EventSupportAging(EventId) == TRUE)
  {
    ...
    if (MemoryIndex != DEM_MEM_INVALID_MEMORY_INDEX)
    {
      ...
      Dem_MemoryEntry_SetAgingCounter(Dem_MemState_GetMemoryEntryId(MemoryIndex), DEM_OPERATIONCYCLE_CYCLECOUNT_AGED);
#else
      {
        Dem_MemState_MemoryInfoPtrType lMemoryInfo;
...
# endif
  ...
  else
  {
    if (MemoryIndex != DEM_MEM_INVALID_MEMORY_INDEX)
    {
      ...
      Dem_MemoryEntry_SetAgingCounter(Dem_MemState_GetMemoryEntryId(MemoryIndex), DEM_OPERATIONCYCLE_CYCLECOUNT_LATCHED);
      Dem_Nvm_SetEventMemoryBlockState(MemoryIndex, DEM_NVM_BLOCKSTATE_DIRTY);
    }
  }
}                                                                                                                                /* PRQA S 6050, 6080 */ /* MD_MSR_STCAL, MD_MSR_STMIF */
  ...
  )
{
  ...
  { /* Already stored - Update the existing entry if necessary */
    ...
    if (Dem_DTCInternalStatus_GetStoredStatus(lInternalStatus) == Dem_DTCInternalStatus_StoredStatus_Aged)
    {
      ...
      { /* Reclaim aged event */
        lIsOccurrence = TRUE;
        lBlockModification = Dem_MemStorageManager_UpdateEventMemoryStatistics(
...
# if (DEM_CFG_DATA_OCCCTR_2BYTE == STD_ON)
    lOldOccurrenceCount = Dem_MemoryEntry_GetOccurrenceCounter2Byte(Dem_MemState_GetMemoryEntryId(lMemoryIndex));
# else
    lOldOccurrenceCount = Dem_MemoryEntry_GetOccurrenceCounter(Dem_MemState_GetMemoryEntryId(lMemoryIndex));
# endif
#endif
#if (DEM_CFG_DATA_CYCLES_SINCE_FIRST_FAILED == STD_ON)
    {
      ...
      if (Dem_MemoryEntry_GetCyclesSinceFirstFailed(Dem_MemState_GetMemoryEntryId(lMemoryIndex)) == DEM_OPERATIONCYCLE_CYCLECOUNT_INVALID)
      {
        Dem_MemoryEntry_SetCyclesSinceFirstFailed(Dem_MemState_GetMemoryEntryId(lMemoryIndex), Dem_OperationCycle_GetCycleCounter(Dem_Cfg_EventOperationCycle(DTCContext->EventId)));
...
# if (DEM_CFG_SUPPORT_OBDII == STD_ON)
    Dem_MemStorageManager_TriggerObdIIFreezeFrameStorage(DTCContext->EventId, 
                                                         DTCContext->DtcEffects);
# endif
# if (DEM_CFG_SUPPORT_WWHOBD == STD_ON)
    lBlockModification |= Dem_MemStorageManager_TriggerWwhObdFreezeFrameStorage(
                          DTCContext->EventId, DTCContext->DtcEffects, 
                          lMemoryIndex);
# endif
  }
#endif

  lBlockModification |= Dem_MemStorageManager_UpdateEventEntry(
                          DTCContext->EventId, DTCContext->DtcEffects, lMemoryIndex);
...
# if (DEM_CFG_DATA_OCCCTR_2BYTE == STD_ON)
    lNewOccurrenceCount = Dem_MemoryEntry_GetOccurrenceCounter2Byte(Dem_MemState_GetMemoryEntryId(lMemoryIndex));
# else
    lNewOccurrenceCount = Dem_MemoryEntry_GetOccurrenceCounter(Dem_MemState_GetMemoryEntryId(lMemoryIndex));
# endif
...
#  if (DEM_CFG_AGING_RETAIN_MEMORY_ENTRY == STD_ON) || (DEM_FEATURE_NEED_AGING_REALLOCATION == STD_ON)
    if (
#   if (DEM_CFG_AGING_RETAIN_MEMORY_ENTRY == STD_ON)
         (Dem_MemoryEntry_GetAgingCounter(Dem_MemState_GetMemoryEntryId(lMemoryIndex)) == DEM_OPERATIONCYCLE_CYCLECOUNT_AGED)
#   endif
#   if (DEM_CFG_AGING_RETAIN_MEMORY_ENTRY == STD_ON) && (DEM_FEATURE_NEED_AGING_REALLOCATION == STD_ON)
      ||
#   endif
#   if (DEM_FEATURE_NEED_AGING_REALLOCATION == STD_ON)
         (DEM_MEM_TEST_AGING_ONLY(Dem_MemoryEntry_GetState(Dem_MemState_GetMemoryEntryId(lMemoryIndex))) == TRUE)
#   endif
      )
    {
      Dem_MemoryEntry_InitializeEntry(Dem_MemState_GetMemoryEntryId(lMemoryIndex));
    }
#  endif
# endif
  }
...
# endif
  ...
  if (lMemoryIndex != DEM_MEM_INVALID_MEMORY_INDEX)
  {
    ...
    lBlockModification = Dem_MemStorageManager_UpdateEventEntry(
                         EventId, lUpdateFlags, lMemoryIndex);
