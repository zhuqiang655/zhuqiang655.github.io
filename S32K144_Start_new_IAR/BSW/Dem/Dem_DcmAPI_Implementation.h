...
# if (DEM_CFG_SUPPORT_OBDII == STD_ON)
    ...
    if (lSelectedIndex != Dem_Cfg_GlobalObdIIFFCount())
    {
      ...
      if (*BufSize >= (Dem_Cfg_GlobalObdIIFFUdsSize() - 1)) /* -1 as RecordNumber is not taken into account here */
      {
        ...
        Dem_DataReportIF_ObdIIFreezeFrameCopyUdsData(lSelectedIndex, &lDestinationBuffer);                                       /* SBSW_DEM_POINTER_LOCAL_DESTINATIONBUFFER */
        /* verify that the FreezeFrame EventId is still the same and valid */
        if ( (Dem_Cfg_EventUdsDtc(Dem_Mem_FreezeFrameGetEvent(lSelectedIndex)) == lUdsDtc)                                       /* PRQA S 3415 */ /* MD_DEM_12.4_cf */
...
# endif
      {
        ...
        if ( (Dem_Mem_FreezeFrameGetEvent(lSelectedIndex) != lFreezeFrameEventId)                                                /* PRQA S 3415 */ /* MD_DEM_12.4_cf */
          && (Dem_Mem_FreezeFrameGetEvent(lSelectedIndex) != DEM_EVENT_INVALID) )
