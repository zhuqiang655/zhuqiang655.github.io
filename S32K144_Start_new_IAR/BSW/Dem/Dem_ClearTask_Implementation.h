  ...
  )
{
  ...
  lMemoryInfo = Dem_MemState_MemoryInfoInit(Dem_ClearTask_GetClearMemoryId());
  ...
  { /* Clear group of events */
    ...
    { /* Not yet started */
      Dem_ClearTask_ClearEventIterator = Dem_MemState_MemoryGetLastEvent(lMemoryInfo);                                           /* SBSW_DEM_CALL_MEMORYINFO_POINTER */
...
# endif
    ...
    { /* At most delete all events to prevent overflow */ 
      lEventLowerLimit = (uint16)(Dem_MemState_MemoryGetFirstEvent(lMemoryInfo) - 1);                                            /* SBSW_DEM_CALL_MEMORYINFO_POINTER */
    }
    else
    {
...
# if (DEM_CFG_SUPPORT_SECONDARY == STD_ON)
      if (lEventLowerLimit < Dem_MemState_MemoryGetFirstEvent(lMemoryInfo))                                                      /* SBSW_DEM_CALL_MEMORYINFO_POINTER */
      {
        lEventLowerLimit = (uint16)(Dem_MemState_MemoryGetFirstEvent(lMemoryInfo) - 1);                                          /* SBSW_DEM_CALL_MEMORYINFO_POINTER */
      }
# endif
...
# if (DEM_CFG_SUPPORT_J1939_CLEAR_DTC == STD_ON)
    ...
    {
      /* Attention: lEventLowerLimit is numerically smaller than current iterator */
      Dem_ClearTask_ProcessClearJ1939Dtc(Dem_MemState_MemoryGetFirstEvent(lMemoryInfo), Dem_MemState_MemoryGetLastEvent(lMemoryInfo));     /* SBSW_DEM_CALL_MEMORYINFO_POINTER */ /* SBSW_DEM_CALL_MEMORYINFO_POINTER */
    }
    else
# endif
    {
      /* Attention: lEventLowerLimit is numerically smaller than current iterator */
      Dem_ClearTask_ProcessClearDtcGroup(Dem_MemState_MemoryGetFirstEvent(lMemoryInfo), Dem_MemState_MemoryGetLastEvent(lMemoryInfo));     /* SBSW_DEM_CALL_MEMORYINFO_POINTER */ /* SBSW_DEM_CALL_MEMORYINFO_POINTER */
    }
#endif
#if (DEM_CFG_SUPPORT_CLEAR_DTC_RUNTIME_LIMIT == STD_ON)
    if (Dem_ClearTask_ClearEventIterator >= Dem_MemState_MemoryGetFirstEvent(lMemoryInfo))                                       /* SBSW_DEM_CALL_MEMORYINFO_POINTER */
    {
      lClearSuccess = FALSE;
    }
    else
