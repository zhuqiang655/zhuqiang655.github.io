...
#if (  (ECUM_SW_MAJOR_VERSION != (0x09u)) \
    || (ECUM_SW_MINOR_VERSION != (0x02u)) \
...
/* vendor specific version information is BCD coded */
#if (  (ECUM_CBK_SW_MAJOR_VERSION != (0x09u)) \
    || (ECUM_CBK_SW_MINOR_VERSION != (0x02u)) \
...
ECUM_LOCAL FUNC(void, ECUM_CODE) EcuM_ShutdownSynchronization(EcuM_CoreStatusType ShutdownState)
{
  ...
  do
  {
    ...
    {
      if (EcuM_GetCoreStatus(loopCount) != ShutdownState)
      {
...
ECUM_LOCAL FUNC(EcuM_SizeOfPartitionInitType, ECUM_CODE) EcuM_InternalGetPartitionIdx(void)
{
  ...
  /* ----- Implementation ----------------------------------------------- */
    ...
    for(partitionIndex = 0; partitionIndex < EcuM_GetSizeOfPartitionInit(); partitionIndex++) /* COV_ECUM_PARTITIONID */
    {
...
# if(ECUM_NUMBER_OF_CORES > 1)
      if(coreID == ECUM_CORE_ID_STARTUP) /* PRQA S 3355, 3356, 3359, 3358 */ /* MD_ECUM_3355_3358_3356_3359 */ /* COV_ECUM_COREID_STARTUP */
# endif
      { /* PRQA S 3201 */ /* MD_ECUM_3201 */
        EcuM_InternalDeterminePbConfiguration();
      }
...
# if((ECUM_CONFIGURATION_VARIANT == ECUM_CONFIGURATION_VARIANT_POSTBUILD_LOADABLE) || (ECUM_POSTBUILD_VARIANT_SUPPORT == STD_ON))
      if(coreID == ECUM_CORE_ID_STARTUP) /* PRQA S 3356, 3359, 3201 */ /* MD_ECUM_3355_3358_3356_3359 */ /* COV_ECUM_COREID_STARTUP */ 
      { /* PRQA S 3201 */ /* MD_ECUM_3201 */
        EcuM_InternalDeterminePbConfiguration();
      }
# endif /* ((ECUM_CONFIGURATION_VARIANT == ECUM_CONFIGURATION_VARIANT_POSTBUILD_LOADABLE) || (ECUM_POSTBUILD_VARIANT_SUPPORT == STD_ON)) */
...
#if(ECUM_SLAVE_CORE_HANDLING == STD_ON)
        EcuM_SetCoreStatus(ECUM_CORE_ID_BSW, ECUM_CORESTATUS_OFF); /* SBSW_SETCORESTATUS_BSWCOREID */
...
# if(ECUM_SLAVE_CORE_HANDLING == STD_ON)
        {
          CoreIdType coreID = GetCoreID();
          EcuM_EnterCriticalArea_1();
          EcuM_SetCoreStatus(coreID, ECUM_CORESTATUS_OFF); /* SBSW_SETCORESTATUS_OSCOREID */
          EcuM_LeaveCriticalArea_1();
        }
# endif
      }
...
*
     ...
     MD_ECUM_3201:
     Reason:     The OS symbol OS_CORE_ID_MASTER which marks the master core is only available as ENUM and not via preprocessor so it can't be encapsulated.
     Risk:       There is no risk, most compilers will optimize thise.
     Prevention: Covered by code review.
