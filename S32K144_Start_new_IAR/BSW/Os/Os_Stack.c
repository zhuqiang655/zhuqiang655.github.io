...
)
{
  ...
  else
  {
    ...
    if(OS_LIKELY(Os_CoreAsrIsCoreLocal(targetCore) != 0))                                                               /* COV_OS_HALPLATFORMMULTICOREUNSUPPORTED */ /* SBSW_OS_ST_COREASRISCORELOCAL_001 */
    {
      P2CONST(Os_StackConfigType, AUTOMATIC, OS_CONST) stack;
...
)
{
  ...
  else
  {
    ...
    else
    {
      ...
      /* #20 Get the stack object and determine the stack usage. */
      if(OS_LIKELY(Os_CoreAsrIsCoreLocal(targetCore) != 0))                                                             /* COV_OS_HALPLATFORMMULTICOREUNSUPPORTED */ /* SBSW_OS_ST_COREASRISCORELOCAL_002 */
      {
        P2CONST(Os_StackConfigType, AUTOMATIC, OS_CONST) stack;
...
)
{
  ...
  else
  {
    ...
    else
    {
      ...
      else if(OS_LIKELY(Os_CoreAsrIsCoreLocal(targetCore) != 0))                                                        /* COV_OS_HALPLATFORMMULTICOREUNSUPPORTED */ /* SBSW_OS_ST_COREASRISCORELOCAL_002 */
      {
        P2CONST(Os_StackConfigType, AUTOMATIC, OS_CONST) stack;
...
\ID SBSW_OS_ST_HAL_STACKENDGET_003
 \DESCRIPTION    Os_Hal_StackEndGet is called with LowAddress and HighAddress of a Stack object. The stack object is
                 the return value of the function Os_CoreGetCurrentStack. Precondition ensures that the pointer is valid.
