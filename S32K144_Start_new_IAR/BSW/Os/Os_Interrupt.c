...
))
{
  return Os_ErrServiceProtectionCheck((Os_StdReturnType)((
                                                  ...
                                                  InterruptState->Os_IntLockingState.SuspendOsCounter) == 0));
}
...
{
  return Os_ErrServiceProtectionCheck((Os_StdReturnType)(InterruptState->Os_IntLockingState.InDisableAll == 1));
}
...
{
  return Os_ErrServiceProtectionCheck((Os_StdReturnType)(InterruptState->Os_IntLockingState.InDisableAll == 0));
}
...
{
  return Os_ErrServiceProtectionCheck((Os_StdReturnType)(InterruptState->Os_IntLockingState.SuspendAllCounter != 0));
}
...
{
  return Os_ErrServiceProtectionCheck((Os_StdReturnType)(InterruptState->Os_IntLockingState.SuspendOsCounter != 0));
}
...
))
{
  ...
  if(Os_Hal_IntIsLevelSupported() != 0)                                                                                 /* COV_OS_HALPLATFORMLEVELSUPPORT */
  {
    ...
    if(Os_Hal_IntLevelIsGt(&cat2DisableLevel, &levelState) != 0)                                                        /* SBSW_OS_FC_POINTER2LOCAL */
    {
...
)
{
  ...
  else
  {
    ...
    if(interruptApiState->Os_IntLockingState.SuspendAllCounter == 0)
    {
...
)
{
  ...
  else
  {
    ...
    if (interruptApiState->Os_IntLockingState.SuspendOsCounter == 0)
    {
      /* #30 Tell timing protection module to start measuring locking time. */
...
)
{
  ...
  {
    /* #50 Otherwise disable global interrupt handling. */
    Os_Api_DisableAllInterrupts();
  }
}
...
)
{
  ...
  if(Os_Hal_IntIsLevelSupported() != 0)                                                                                 /* COV_OS_HALPLATFORMLEVELSUPPORT */
  {
    ...
    /* #20 Inform the trace module that interrupts are going to be enabled. */
    Os_TraceInterruptsLevelEnabled();
    ...
    /* #40 Tell timing protection module to stop measuring locking time. */
    Os_InterruptTpLockOsStop(Os_CoreGetThread());                                                                       /* SBSW_OS_INT_INTERRUPTTPLOCKOSSTOP_001 */
  }
  else
  {
    /* #50 Otherwise enable global interrupt handling. */
    Os_Api_EnableAllInterrupts();
  }
}
