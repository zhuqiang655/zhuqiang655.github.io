...
)
{
  ...
  if(OS_LIKELY(Os_TpCheckInterArrival(timingProtection, Os_ThreadGetTpConfig(&Isr->Thread)) != OS_CHECK_FAILED))        /* COV_OS_HALPLATFORMTPUNSUPPORTED_TX */ /* SBSW_OS_ISR_TPCHECKINTERARRIVAL_001 */ /* SBSW_OS_FC_PRECONDITION */
  {
...
OS_FUNC_ATTRIBUTE_DEFINITION(void, OS_CODE, OS_NORETURN, Os_IsrEpilogue, (void))
{
  ...
  if(Os_ThreadIsTask(interruptedThread) != 0)                                                                           /* COV_OS_HALPLATFORMISRNESTINGSUPPORT_TX */ /* SBSW_OS_ISR_THREADISTASK_001 */
  {
