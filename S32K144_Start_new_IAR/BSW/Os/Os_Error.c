...
{
  return (Os_StdReturnType)(OS_CFG_INTERNAL_EXCEPTION_DETECTION == STD_ON);
}
...
{
  return (Os_StdReturnType) ((Address >= Section->StartLabel) &&
...
)
{
  ...
  else if(Os_ErrIsInternalExceptionDetectionSupported() != 0)                                                           /* COV_OS_ERRCODESECTIONCHECK */
  {
    Os_ObjIdxType index;
    result = 0;
    ...
    for(index = 0; index < (Os_ObjIdxType)OS_CFG_NUM_OSCODE_SECTIONS; index++)
    {
      ...
      if (Os_ErrIsAddressInsideSection(OsCfg_OsCode_Sections[index], ExceptionInstructionAddress) != 0)                 /* SBSW_OS_ERROR_ISADDRESSINSIDESECTION_001 */
      {
...
)
{
  ...
  if (Os_CoreCriticalUserSectionIsActive(Os_ThreadGetCore(currentThread)) != 0)                                         /* SBSW_OS_ERROR_THREADGETCORE_001 */ /* SBSW_OS_ERROR_CORECRITICALUSERSECTIONISACTIVE_001 */
  {
    Os_ErrKernelPanic();
  }
  /* #20 Otherwise, if the OS is potentially in an inconsistent state, call the panic hook. */
  else if(Os_ErrIsOsInternalException(ExceptionInstructionAddress, ExceptionCauserPrivileged) != 0)                     /* SBSW_OS_ERROR_ISOSINTERNALEXCEPTION */
  {
    Os_ErrKernelPanic();
  }
