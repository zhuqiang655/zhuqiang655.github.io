...
[!LOOP "GptHwConfiguration/*"!]
    ...
    [!IF "($IPVType = 'FTM' or $IPVType = 'LPIT') and (ecu:get(concat('Gpt.GptConfig.Gpt_', $IPVType,'_ModuleSingleInterrupt'))='STD_ON')"!]
    [!IF "$ModuleIdx != substring-before(GptIsrHwId, '_CH')"!]
[!NOCODE!]
[!CODE!]
#define GPT_[!"substring-before(GptIsrHwId, '_CH')"!]_ISR_USED
[!ENDCODE!]
[!ENDNOCODE!]
    [!VAR "ModuleIdx" = "substring-before(GptIsrHwId, '_CH')"!] 
    [!ENDIF!]
    [!ELSE!]
[!NOCODE!]
[!CODE!]
