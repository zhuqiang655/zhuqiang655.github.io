                ...
                switch(au8Lin_Command)
                {
                    ...
                    case LIN_TX_SLAVE_RES_COMMAND:
                        ...
                        if(u8BufferPtr == (u8BufferLength - (uint8)1U))
                        {
                            ...
                            u8Lin_FrameID = au8LPUART_Buffer[u8LogicalChannel][LPUART_PID_BYTE_OFFSET_U8];

                            /* Checksum Error */
                            if((LPUART_MASTER_REQUEST_DIAGNOSTIC_ID_U8 == u8Lin_FrameID) | (LPUART_SLAVE_RESPONSE_DIAGNOSTIC_ID_U8 == u8Lin_FrameID))
                            {
                                u8Lin_Checksum = Lin_LPUART_ChecksumCalc((uint8)u8LogicalChannel, LIN_CLASSIC_CS);
                            }
                            else
                            {
                                u8Lin_Checksum = Lin_LPUART_ChecksumCalc((uint8)u8LogicalChannel, LIN_ENHANCED_CS);
                            }

                            if(u8Lin_Checksum != (uint32)(u32Lin_DataReg & LPUART_DATA_8BITS_MASK_U32))
                            {
